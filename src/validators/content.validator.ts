...
export const createContentSchema =
    zfd.formData({
        ...
        description: zfd.text(z.string().min(1, "Description is needed")),
        language: zfd.text(z.enum(['english', 'marathi', 'hindi'])),
        type: zfd.text(z.enum(['daily', 'educational'])),
        hidden: zfd.text(z.enum(['true', 'false']).transform(val => val === 'true'))
    })
